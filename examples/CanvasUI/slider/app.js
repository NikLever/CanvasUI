...
class App{
    ...
    createUI() {
        const config = {
            ...
            height: 256,
            slider: { type: "slider", position: { left: 20, top: 20 }, width:472, height:88, onChange },
            info: { type: "text", position: { left: 20, top: 128 }, fontSize:18, },
            opacity: 1,
            renderer: this.renderer
        }
        const content = {
            slider: 1,
            info: "Use the slider to size the ball"
        }
        ...
        function onChange(val){
            const msg = `Slider changed to (${val})`;
            console.log(msg);
            self.ball.scale.set( val, val, val );
        }
    }
